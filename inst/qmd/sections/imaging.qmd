...
count_imaging <- function(
  ...
  atri_imaging <-
    purrr::map(
      abcdsReporter:::.imaging_key[["atri_scan_names"]],
      .f = function(x) {
        img <- abcdsReporter::get_imaging(
          done,
          imaging = x,
          controls = controls
        )
  ...
  if (!is.null(lonidir)) {
    loni_imaging <- purrr::map(
      abcdsReporter:::.imaging_key[["loni_scan_names"]],
      .f = function(x) {
  ...
  imaging_scan_counts <- abcdsReporter::multiple_count_by(
    data = atri_imaging,
    names = paste0(abcdsReporter:::.imaging_key[["loni_scan_names"]], "_done"),
    labels = abcdsReporter:::.imaging_key[["scan_labels"]],
    ids = "ids",
...
count_mri_sequencing <- function(controls = FALSE, lonidir = NULL) {
  atri_mri_imaging <- abcdsReporter::get_mri_sequences(
    name = "description",
    controls = controls,
    apply_labels = TRUE
  )

...
cognition <- purrr::map2(
  ...
  .f = function(x, y) {
    cog <- abcdsReporter::get_cognition(done, task = x)
    dplyr::rename(cog, !!y := "done")
  }
)

id_vars <- abcdsReporter:::get_ids(cognition[[1]])
...
if (!is.null(params$lonidir)) {
  ...
  atri_cognition <- abcdsReporter:::loni_join(
...
abcdsReporter:::format_quarto(
  "Number of Participants with Completed Cognition Tasks",
  "subsection"
)

cognition_tasks <- abcdsReporter::multiple_count_by(
  data = atri_cognition,
